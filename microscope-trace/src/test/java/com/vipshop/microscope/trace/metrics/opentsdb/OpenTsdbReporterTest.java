...
public class OpenTsdbReporterTest {
	...
	private MicroscopeReporter reporter;
	...
	public void setUp() throws Exception {
		...
		reporter = MicroscopeReporter.forRegistry(registry).withClock(clock).convertRatesTo(TimeUnit.SECONDS).convertDurationsTo(TimeUnit.MILLISECONDS).filter(MetricFilter.ALL)
				.build();
	...
	public void testReportGauges() {
		...
		final Set<Metric> metrics = captor.getValue();
	...
	public void testReportHistogram() {
		...
		final Metric metric = metrics.iterator().next();
		assertEquals((Long) timestamp, metric.getTimestamp());
