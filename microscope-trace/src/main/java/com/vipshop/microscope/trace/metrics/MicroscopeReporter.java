...
public class MicroscopeReporter extends ScheduledReporter {
    ...
    private void buildGauge(String name, Gauge gauge, long timestamp) {
        ...
        output.addMetrics(MetricData.named(prefix(name))
    ...
    private void buildHistograms(String name, Histogram histogram, long timestamp) {
        ...
        output.addMetrics(MetricData.named(prefix(name, "count"))
                .withTimestamp(timestamp)
                .withValue(histogram.getCount())
                .withTags(tags).build());

        output.addMetrics(MetricData.named(prefix(name, "max"))
                .withTimestamp(timestamp)
                .withValue(snapshot.getMax())
                .withTags(tags).build());

        output.addMetrics(MetricData.named(prefix(name, "min"))
                .withTimestamp(timestamp)
                .withValue(snapshot.getMin())
                .withTags(tags).build());

        output.addMetrics(MetricData.named(prefix(name, "mean"))
                .withTimestamp(timestamp)
                .withValue(snapshot.getMean())
                .withTags(tags).build());

        output.addMetrics(MetricData.named(prefix(name, "stddev"))
                .withTimestamp(timestamp)
                .withValue(snapshot.getStdDev())
                .withTags(tags).build());


        output.addMetrics(MetricData.named(prefix(name, "p50"))
                .withTimestamp(timestamp)
                .withValue(snapshot.getMedian())
                .withTags(tags).build());

        output.addMetrics(MetricData.named(prefix(name, "p75"))
                .withTimestamp(timestamp)
                .withValue(snapshot.get75thPercentile())
                .withTags(tags).build());

        output.addMetrics(MetricData.named(prefix(name, "p95"))
                .withTimestamp(timestamp)
                .withValue(snapshot.get95thPercentile())
                .withTags(tags).build());


        output.addMetrics(MetricData.named(prefix(name, "p98"))
                .withTimestamp(timestamp)
                .withValue(snapshot.get98thPercentile())
                .withTags(tags).build());

        output.addMetrics(MetricData.named(prefix(name, "p99"))
                .withTimestamp(timestamp)
                .withValue(snapshot.get99thPercentile())
                .withTags(tags).build());

        output.addMetrics(MetricData.named(prefix(name, "p999"))
                .withTimestamp(timestamp)
    ...
    private void buildMeters(String name, Meter meter, long timestamp) {
        ...
        output.addMetrics(MetricData.named(prefix(name, "meanrate"))
                .withTimestamp(timestamp)
                .withValue(meter.getMeanRate())
                .withTags(tags).build());

        output.addMetrics(MetricData.named(prefix(name, "1-meanrate"))
                .withTimestamp(timestamp)
                .withValue(meter.getOneMinuteRate())
                .withTags(tags).build());

        output.addMetrics(MetricData.named(prefix(name, "5-meanrate"))
                .withTimestamp(timestamp)
                .withValue(meter.getFiveMinuteRate())
                .withTags(tags).build());

        output.addMetrics(MetricData.named(prefix(name, "15-meanrate"))
                .withTimestamp(timestamp)
    ...
    private void buildHealths(String name, HealthCheck.Result result, long timestamp) {
        ...
        output.addMetrics(MetricData.named(prefix(name, "health"))
                .withTimestamp(timestamp)
