...
//		Repositorys.TRAC_REPOSITORY.drop();
	...
	public void testFindAll() throws IOException {
		...
		HbaseFactory.TRACE.save(Arrays.asList(trace1, trace2, trace3));
		HbaseFactory.TRACE.delete(trace1);
		List<TraceTable> traces = HbaseFactory.TRACE.findAll();
	...
	public void testFindScan() throws IOException {
		...
		HbaseFactory.TRACE.save(Arrays.asList(trace1, trace2, trace3, trace4, trace5, trace6));

		Scan scan = new Scan(Bytes.toBytes("xufei-1"), Bytes.toBytes("xufei-5"));
		List<TraceTable> traces = HbaseFactory.TRACE.findWithScan(scan);
		Assert.assertEquals(4, traces.size());
		
		scan = new Scan(Bytes.toBytes("xufei-1"));
		traces = HbaseFactory.TRACE.findWithScan(scan);
