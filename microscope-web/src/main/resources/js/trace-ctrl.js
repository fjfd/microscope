...
angular.module('microscope.app.trace', ['ui.validate', 'microscope.app.filters'])

    .controller('traceCtrl', ['$rootScope', '$scope', '$http', 'serverConfig',
        function ($rootScope, $scope, $http, serverConfig) {

            // 页面进入时首次请求
            $http.jsonp('http://' + serverConfig.hostname + ':' + serverConfig.port + '/trace/apps?callback=JSON_CALLBACK')
                .success(function (data) {
                    ...
                    // if (queryList && queryList.app) {
                    //     $scope.appObjList = queryList.app;
                    //     for (var i = 0; i < $scope.appObjList.length; i++) {
                    //         appList.push($scope.appObjList[i].app);
                    //     }
                    //     $scope.appList = appList;
                    ...
                    // $scope.queryApp = $scope.appList[0];
                    // $scope.bindTrace();
                    // $scope.resultList = queryList['traceList'];
                });

            $scope.bindTrace = function () {
                for (var i = 0; i < $scope.appObjList.length; i++) {
                    if ($scope.appObjList[i].app === $scope.queryApp) {
                        $scope.traceList = $scope.appObjList[i].trace;
            ...
            $scope.query = function () {
                ...
                today = new Date();
                $scope.queryStartTime = today.format('hh:mm:ss');
                $scope.queryEndTime = today.format('hh:mm:ss');
                $http.jsonp('http://' + serverConfig.hostname + ':' + serverConfig.port + '/trace/traceList?' +
                        'appName=' + $scope.queryApp +
                        '&ipAddress=' + $scope.queryIP +
                        '&traceName=' + encodeURIComponent($scope.queryTrace || '') +
                    ...
                    .success(function (data) {
                        for (var i = 0, l = data.result.length; i < l; i++) {
                            data.result[i].startTimestamp = parseInt(data.result[i].startTimestamp);
                            data.result[i].endTimestamp = parseInt(data.result[i].endTimestamp);
                            data.result[i].durationMicro = parseInt(data.result[i].durationMicro);
                        }
    ...
    .controller('traceDetailCtrl', ['$rootScope', '$scope', '$http', 'serverConfig',
        function ($rootScope, $scope, $http, serverConfig) {
            $http.jsonp('http://' + serverConfig.hostname + ':' + serverConfig.port + '/trace/traceSpan?' +
                    'traceId=' + $rootScope.$stateParams.traceId +
                    '&callback=JSON_CALLBACK')
                .success(function (data) {
                    ...
                    var treeHtml = createTree(tree, 0);
                    treeHtml += '<script type="text/javascript">bindGanttEvent();</script>';
                    document.getElementById('gantt-tree-wrap').innerHTML = treeHtml;


                    var totalTtime = ganttItem[0].duration,
                        parentSrarttime = ganttItem[0].start_time;

                    for (var i = 0, l = ganttItem.length; i < l; i++) {
                        ganttItem[i].style = {
                            width: ganttItem[i].duration === 0 ? '1%' : ganttItem[i].duration / totalTtime * 100 + '%',
                            left: (ganttItem[i].start_time - parentSrarttime) / totalTtime * 100 + '%'
                        };
                        ganttItem[i].start_time = new Date(parseInt(ganttItem[i].start_time)).format('yyyy-MM-dd hh:mm:ss');
                    }

                    $scope.ganttItem = ganttItem;
                });
        }
    ]);