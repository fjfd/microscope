...
public class HiveApp {
	...
	public static void main(String[] args) throws Exception {
		context = new ClassPathXmlApplicationContext("/META-INF/spring/hive-context.xml", HiveApp.class);
		log.info("Hive Application Running");
		context.registerShutdownHook();

		HiveTemplate template = context.getBean(HiveTemplate.class);
		log.info(template.query("show databases;"));
		
		log.info(template.query("select * from test.h_trace"));
