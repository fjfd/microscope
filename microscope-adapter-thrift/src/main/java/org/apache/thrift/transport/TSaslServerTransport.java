...
public class TSaslServerTransport extends TSaslTransport {
	 ...
	 * protocol. Receives the initial response from the client, creates a SASL
	 * server using the mechanism requested by the client (if this server
	 * supports it), and sends the first challenge back to the client.
	 */
	@Override
	...
	public static class Factory extends TTransportFactory {
		 ...
		 */
		private static Map<TTransport, WeakReference<TSaslServerTransport>> transportMap = Collections.synchronizedMap(new WeakHashMap<TTransport, WeakReference<TSaslServerTransport>>());
		 ...
		 * created before using the given <code>TTransport</code> as an
		 * underlying transport. This ensures that a given underlying transport
		 * instance receives the same <code>TSaslServerTransport</code>.
		 */
		@Override
		public TTransport getTransport(TTransport base) {
			...
			if (ret == null || ret.get() == null) {
				...
				transportMap.put(base, ret); // No need for putIfAbsent().
												// Concurrent calls to
												// getTransport() will pass in
												// different TTransports.
			} else {
