...
/**
 * FileTransport implementation of the TTransport interface. Currently this is a
 * straightforward port of the cpp implementation
 * 
 * It may make better sense to provide a basic stream access on top of the
 * framed file format The FileTransport can then be a user of this framed file
 * format with some additional logic for chunking.
 * 
...
public class TFileTransport extends TTransport {
	...
	public static enum tailPolicy {
		...
		/**
		 * Number of retries before giving up if 0, no retries if -1, retry
		 * forever
		 */
	...
	/**
	 * Diverging from the cpp model and sticking to the TSocket model Files are
	 * not opened in ctor - but in explicit open call
	 */
	...
	/**
	 * Cloned from TTransport.java:readAll(). Only difference is throwing an EOF
	 * exception where one is detected.
	 */
