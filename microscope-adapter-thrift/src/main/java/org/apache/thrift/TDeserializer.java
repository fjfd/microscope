...
public class TDeserializer {
	...
	public void partialDeserialize(TBase tb, byte[] bytes, TFieldIdEnum fieldIdPathFirst, TFieldIdEnum... fieldIdPathRest) throws TException {
		try {
			if (locateField(bytes, fieldIdPathFirst, fieldIdPathRest) != null) {
				// if this line is reached, iprot will be positioned at the
				// start of tb.
				tb.read(protocol_);
			}
		} catch (Exception e) {
	...
	/**
	 * Deserialize only a boolean field (addressed by recursively using field
	 * id) from a byte record.
	 * 
	 * @param bytes
	 *            The serialized object to read from
	 * @param fieldIdPathFirst
	...
	/**
	 * Deserialize only the id of the field set in a TUnion (addressed by
	 * recursively using field id) from a byte record.
	 * 
	 * @param bytes
	 *            The serialized object to read from
	 * @param fieldIdPathFirst
	...
	public Short partialDeserializeSetFieldIdInUnion(byte[] bytes, TFieldIdEnum fieldIdPathFirst, TFieldIdEnum... fieldIdPathRest) throws TException {
		try {
			...
			if (field != null) {
				...
				return protocol_.readFieldBegin().id; // The field set in the
														// union
			}
			return null;
		} catch (Exception e) {
	...
	private Object partialDeserializeField(byte ttype, byte[] bytes, TFieldIdEnum fieldIdPathFirst, TFieldIdEnum... fieldIdPathRest) throws TException {
		try {
			TField field = locateField(bytes, fieldIdPathFirst, fieldIdPathRest);
			if (field != null) {
				// if this point is reached, iprot will be positioned at the
				// start of the field.
				switch (ttype) {
				case TType.BOOL:
	...
	private TField locateField(byte[] bytes, TFieldIdEnum fieldIdPathFirst, TFieldIdEnum... fieldIdPathRest) throws TException {
		...
		while (curPathIndex < fieldIdPath.length) {
			...
			// we can stop searching if we either see a stop or we go past the
			// field
			// id we're looking for (since fields should now be serialized in
			// asc
			// order).
