...
public class TCompactProtocol extends TProtocol {
	...
	/**
	 * The maximum number of bytes to read from the network for variable-length
	 * fields (such as strings or binary) or -1 for unlimited.
	 */
	private final long maxNetworkBytes_;
	...
	private long bytesToLong(byte[] bytes) {
		return ((bytes[7] & 0xffL) << 56) | ((bytes[6] & 0xffL) << 48) | ((bytes[5] & 0xffL) << 40) | ((bytes[4] & 0xffL) << 32) | ((bytes[3] & 0xffL) << 24) | ((bytes[2] & 0xffL) << 16)
