...
import com.vipshop.microscope.storage.opentsdb.stats.Histogram;
...
final class TsdbQuery implements Query {
    ...
    public void setEndTime(final long timestamp) {
        if ((timestamp & 0xFFFFFFFF00000000L) != 0) {
            ...
            throw new IllegalArgumentException("new end time (" + timestamp
                    + ") is less than or equal to start time: "
                    + getStartTime());
        }
        // Keep the 32 bits.
     ...
     *
     * @param tags The tags from which to extract the 'GROUP BY's. Each tag that
     ...
     * Since a {@link Span} actually contains multiple HBase rows, the
     * row key stored in the map has its timestamp zero'ed out.
     * @throws HBaseException           if there was a problem communicating with HBase to perform
     *                                  the search.
    ...
    private TreeMap<byte[], Span> findSpans() throws HBaseException {
        ...
        try {
            ...
            while ((rows = scanner.nextRows().joinUninterruptibly()) != null) {
                ...
                for (final ArrayList<KeyValue> row : rows) {
                    ...
                    if (compacted != null) { // Can be null if we ignored all
                        // KVs.
        ...
        LOG.info(this + " matched " + nrows + " rows in " + spans.size()
                + " spans");
        if (nrows == 0) {
            return null;
        }
     ...
     *              Can be {@code null}, in which case the array returned will be
     *              empty.
    ...
    private DataPoints[] groupByAndAggregate(final TreeMap<byte[], Span> spans) {
        ...
        if (group_bys == null) {
            ...
            final SpanGroup group = new SpanGroup(tsdb, getScanStartTime(),
                    getScanEndTime(), spans.values(), rate, aggregator,
        ...
        for (final Map.Entry<byte[], Span> entry : spans.entrySet()) {
            ...
            // make it much better since both the row key and group_bys are
            // sorted.
            ...
            if (value_id == null) {
                LOG.error("WTF?  Dropping span for row "
                        + Arrays.toString(row)
            ...
            if (thegroup == null) {
                thegroup = new SpanGroup(tsdb, getScanStartTime(),
                        getScanEndTime(), null, rate, aggregator,
                        sample_interval, downsampler);
                // Copy the array because we're going to keep `group' and
                // overwrite
                // its contents. So we want the collection to have an immutable
                // copy.
    ...
    Scanner getScanner() throws HBaseException {
        ...
        // & end dates in order to do proper rate calculation or downsampling
        // near
            ...
            for (final byte[] tag_id : group_bys) {
                ...
                if (group_by_values != null) {
                    ...
                    for (final byte[] value_id : value_ids) {
                        ...
                        buf.append(' ').append(Arrays.toString(value_id))
                                .append(", ");
                    }
                    buf.append('}');
                }
    ...
    private static final class SpanCmp implements Comparator<byte[]> {
        ...
        public int compare(final byte[] a, final byte[] b) {
            ...
            for (i = 0; i < metric_width; i++) {
                if (a[i] != b[i]) {
                    return (a[i] & 0xFF) - (b[i] & 0xFF); // "promote" to
                    // unsigned.
                }
            }
