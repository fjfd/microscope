...
package com.vipshop.microscope.storage.opentsdb.core;
...
import com.vipshop.microscope.storage.opentsdb.stats.StatsCollector;

/**
 * "Queue" of rows to compact.
 * <p>
 * Whenever we write a data point to HBase, the row key we write to is added to
 * this queue, which is effectively a sorted set. There is a separate thread
 * that periodically goes through the queue and look for "old rows" to compact.
 * A row is considered "old" if the timestamp in the row key is older than a
 * certain threshold.
 * <p>
 * The compaction process consists in reading all the cells within a given row
 * and writing them back out as a single big cell. Once that writes succeeds, we
 * delete all the individual little cells.
 * <p>
 * This process is effective because in HBase the row key is repeated for every
 * single cell. And because there is no way to efficiently append bytes at the
 * end of a cell, we have to do this instead.
 */
final class CompactionQueue extends ConcurrentSkipListMap<byte[], Boolean> {

	private static final Logger LOG = LoggerFactory
			.getLogger(CompactionQueue.class);

	/**
	 * How many items are currently in the queue. Because
	 * {@link ConcurrentSkipListMap#size} has O(N) complexity.
	 */
	 ...
	 *         flushed (or something failed, in which case the deferred will
	 *         carry the exception). In case of success, the kind of object
	 *         returned is unspecified.
	 */
	public Deferred<ArrayList<Object>> flush() {
		final int size = size();
		if (size > 0) {
			LOG.info("Flushing all old outstanding rows out of " + size
					+ " rows");
		}
	 ...
	 * @param maxflushes
	 *            How many rows to flush off the queue at once. This integer is
	 *            expected to be strictly positive.
	 * @return A deferred that will be called back once everything has been
	 *         flushed.
	 */
	private Deferred<ArrayList<Object>> flush(final long cut_off, int maxflushes) {
		assert maxflushes > 0 : "maxflushes must be > 0, but I got "
				+ maxflushes;
		...
		final ArrayList<Deferred<Object>> ds = new ArrayList<Deferred<Object>>(
				Math.min(maxflushes, MAX_CONCURRENT_FLUSHES));
		int nflushes = 0;
		...
		for (final byte[] row : this.keySet()) {
			...
			if (base_time > cut_off) {
				...
				// We kicked off the compaction of too many rows already, let's
				// wait
				// until they're done before kicking off more.
				break;
			}
			// You'd think that it would be faster to grab an iterator on the
			// map
			// and then call remove() on the iterator to "unlink" the element
			// directly from where the iterator is at, but no, the JDK
			// implements
			// it by calling remove(key) so it has to lookup the key again
			// anyway.
		...
		if (nflushes == MAX_CONCURRENT_FLUSHES && maxflushes > 0) {
			...
			final class FlushMoreCB implements
					Callback<Deferred<ArrayList<Object>>, ArrayList<Object>> {
				public Deferred<ArrayList<Object>> call(
	 ...
	 */
	private final class CompactCB implements
			Callback<Object, ArrayList<KeyValue>> {
	 ...
	 * 
	 * @param row
	 *            The row containing all the KVs to compact. Must contain at
	 *            least one element.
	 ...
	 * Compacts a row into a single {@link KeyValue}.
	 * <p>
	 * If the {@code row} is empty, this function does literally nothing. If
	 * {@code compacted} is not {@code null}, then the compacted form of this
	 ...
	 * 
	 * @param row
	 *            The row containing all the KVs to compact. Must be non-null.
	 * @param compacted
	 *            If non-null, the first item in the array will be set to a
	 *            {@link KeyValue} containing the compacted form of this row. If
	 *            non-null, we will also not write the compacted form back to
	 *            HBase unless the timestamp in the row key is old enough.
	...
	private Deferred<Object> compact(final ArrayList<KeyValue> row,
		...
		if (row.size() <= 1) {
			if (row.isEmpty()) { // Maybe the row got deleted in the mean time?
				...
				if (qual.length % 2 != 0 || qual.length == 0) {
					// Right now we expect all qualifiers to have an even number
					// of
					// bytes. We only have one KV and it doesn't look valid so
					// just
		...
		final KeyValue compact;
		{
			...
			int qual_len = 0; // Pre-compute the size of the qualifier we'll
								// need.
			...
			for (int i = 0; i < nkvs; i++) {
				...
				// been compacted, potentially partially, so we need to merge
				// the
				...
				if (len != 2) {
					// Right now we expect all qualifiers to have an even number
					// of
					// bytes. If we find one with an odd number of bytes, or an
					// empty
					// qualifier (which is possible), just skip it, we don't
					// know what
					// this is. It could be some junk that somehow got in the
					// table,
					// or it could be something from a future version of
					// OpenTSDB that
					// we don't know how to handle, so silently ignore it in
					// order to
					// help be forward compatible with it.
					if (len % 2 != 0 || len == 0) {
						row.remove(i); // This is O(n) but should happen *very*
										// rarely.
						nkvs--;
						i--;
						continue;
					...
					trivial = false;
					// We only do this here because no qualifier can be < 2
					// bytes.
					...
					final short delta = (short) ((Bytes.getShort(qual) & 0xFFFF) >>> Const.FLAG_BITS);
					// This data point has a time delta that's less than or
					// equal to
					// the previous one. This typically means we have 2 data
					// points
					// at the same timestamp but they have different flags.
					// We're
					// going to abort here because someone needs to fsck the
					// table.
					if (delta <= last_delta) {
						throw new IllegalDataException(
								"Found out of order or duplicate"
					...
					last_delta = delta;
					// We don't need it below for complex compactions, so we
					// update it
					// only here in the `else' branch.
					final byte[] v = kv.value();
					val_len += floatingPointValueToFix(qual[1], v) ? 4
							: v.length;
				}
				qual_len += len;
			}
			...
			if (row.size() < 2) {
				...
				if (row.isEmpty()) {
					...
					// We have only one KV left, we call ourselves recursively
					// to handle
					// the case where this KV is an old, incorrectly encoded
					// floating
					// point value that needs to be fixed. This is guaranteed to
					// not
					// recurse again.
				...
				// Now it's vital that we check whether the compact KV has the
				// same
				// qualifier as one of the qualifiers that were already in the
				// row.
				// Otherwise we might do a `put' in this cell, followed by a
				// delete.
				...
				// qualifier, we can opportunistically check here if it happens
				// to
				...
				if (qual.length <= longest_qual.length) {
					...
					if (Bytes.equals(longest_qual, qual)) {
						...
						// Worst case: to be safe we have to loop again and
						// check all
						// the qualifiers and make sure we're not going to
						// overwrite
						// anything.
						// TODO(tsuna): Try to write a unit test that triggers
						// this code
						// path. I'm not even sure it's possible. Should we
						// replace
						// this code with an `assert false:
					...
					if (dup != null) {
						// So we did find an existing KV with the same
						// qualifier.
						// Let's check if, by chance, the value is the same too.
						if (Bytes.equals(dup.value(), compact.value())) {
							// Since the values are the same, we don't need to
							// write
							// anything. There's already a properly compacted
							// version of
							// this row in TSDB.
							write = false;
						}
						// Now let's make sure we don't delete this qualifier.
						// This
						// re-allocates the entire array, but should be a rare
						// case.
						row.remove(dup_idx);
					} // else: no dup, we're good.
				} // else: most common case: the compact qualifier is longer
					// than
		...
		if (write) {
			...
			return tsdb.put(key, qual, value).addCallbacks(
					new DeleteCompactedCB(row), handle_write_error);
		} else {
	 ...
	 * <p>
	 * This method is to be used only when all the cells in the given row are
	 * individual data points (nothing has been compacted yet). If any of the
	 * cells have already been compacted, the caller is expected to call
	 * {@link #complexCompact} instead.
	 * 
	 * @param row
	 *            The row to compact. Assumed to have 2 elements or more.
	 * @param qual_len
	 *            Exact number of bytes to hold the compacted qualifiers.
	 * @param val_len
	 *            Exact number of bytes to hold the compacted values.
	...
	private static KeyValue trivialCompact(final ArrayList<KeyValue> row,
			final int qual_len, final int val_len) {
	 ...
	 * floating point values (pretending they were encoded on 8 bytes when in
	 * fact they were on 4). So overwrite these bits here to make sure they're
	 * correct now, because once they're compacted it's going to be quite hard
	 * to tell if the flags are right or wrong, and we need them to be correct
	 * to easily decode the values.
	 * 
	 * @param flags
	 *            The least significant byte of a qualifier.
	 * @param val_len
	 *            The number of bytes in the value of this qualifier.
	 ...
	 *            The least significant byte of a qualifier.
	 * @param value
	 *            The value that may need to be corrected.
	 */
	private static boolean floatingPointValueToFix(final byte flags,
			final byte[] value) {
		return (flags & Const.FLAG_FLOAT) != 0 // We need a floating point
												// value.
				&& (flags & Const.LENGTH_MASK) == 0x3 // That pretends to be on
														// 4 bytes.
			...
			if (value[0] == 0 && value[1] == 0 && value[2] == 0
				...
				throw new IllegalDataException(
						"Corrupted floating point value: "
								+ Arrays.toString(value) + " flags=0x"
	 ...
	 * races involved with TSDs trying to compact the same row at the same time,
	 * or old data being slowly written to a TSD.
	 * 
	 * @param row
	 *            The row to compact. Assumed to have 2 elements or more.
	 * @param estimated_nvalues
	 *            Estimate of the number of values to compact. Used to
	 *            pre-allocate a collection of the right size, so it's better to
	 *            overshoot a bit to avoid re-allocations.
	 * @return a {@link KeyValue} containing the result of the merge of all the
	 *         {@code KeyValue}s given in argument.
	 * @throws IllegalDataException
	 *             if one of the cells cannot be read because it's corrupted or
	 *             in a format we don't understand.
	 */
	static KeyValue complexCompact(final ArrayList<KeyValue> row,
			final int estimated_nvalues) {
		// We know at least one of the cells contains multiple values, and we
		// need
		...
		for (int i = 0; i < ncells; i++) {
			...
			final short delta = (short) ((Bytes.getShort(cell.qualifier) & 0xFFFF) >>> Const.FLAG_BITS);
			// Because we sorted `cells' by qualifier, and because the time
			// delta
			// occupies the most significant bits, this should never trigger.
			assert delta >= last_delta : ("WTF? It's supposed to be sorted: "
					+ cells + " at " + i + " delta=" + delta + ", last_delta=" + last_delta);
			// The only troublesome case is where we have two (or more)
			// consecutive
			// cells with the same time delta, but different flags or values.
			if (delta == last_delta) {
				// Find the previous cell. Because we potentially replace the
				// one
				// right before `i' with a tombstone, we might need to look
				// further
				// back a bit more.
				Cell prev = Cell.SKIP;
				...
				if (cell.qualifier[1] != prev.qualifier[1]
					...
					throw new IllegalDataException(
							"Found out of order or duplicate" + " data: cell="
									+ cell + ", delta=" + delta
									+ ", prev cell=" + prev + ", last_delta="
									+ last_delta + ", in row=" + row
									+ " -- run an fsck.");
				}
				// else: we're good, this is a true duplicate (same qualifier &
				// value).
				// Just replace it with a tombstone so we'll skip it. We don't
				// delete
	 ...
	 *             in a format we don't understand.
	 */
	private static ArrayList<Cell> breakDownValues(
		...
		for (final KeyValue kv : row) {
			...
			if (len == 2) { // Single-value cell.
				...
				if (q != qual[1]) { // We need to fix the qualifier.
					...
					actual_qual = qual; // Otherwise use the one we already
										// have.
				}
			...
			// this compacted cell has been written by a future version of
			// OpenTSDB
			// and we don't know how to decode it, so we shouldn't touch it.
			if (val[val.length - 1] != 0) {
				throw new IllegalDataException(
						"Don't know how to read this value:"
								+ Arrays.toString(val)
								+ " found in "
								+ kv
			...
			// Check we consumed all the bytes of the value. Remember the last
			// byte
			// is metadata, so it's normal that we didn't consume it.
			if (val_idx != val.length - 1) {
				throw new IllegalDataException(
						"Corrupted value: couldn't break down"
								+ " into individual values (consumed "
								+ val_idx + " bytes, but was"
								+ " expecting to consume " + (val.length - 1)
								+ "): " + kv + ", cells so far: " + cells);
			}
		}
		return cells;
	}
	...
	private final HandleErrorCB handle_delete_error = new HandleErrorCB(
			"delete");
	...
	private final class HandleErrorCB implements Callback<Object, Exception> {
		 ...
		 * 
		 * @param what
		 *            String describing what kind of operation (e.g. "read").
		 */
		...
		public Object call(final Exception e) {
			if (e instanceof PleaseThrottleException) { // HBase isn't keeping
														// up.
				final HBaseRpc rpc = ((PleaseThrottleException) e)
						.getFailedRpc();
				if (rpc instanceof HBaseRpc.HasKey) {
					// We failed to compact this row. Whether it's because of a
					// failed
					// get, put or delete, we should re-schedule this row for a
					// future
					// compaction.
					add(((HBaseRpc.HasKey) rpc).key());
					return Boolean.TRUE; // We handled it, so don't return an
											// exception.
			...
			while (true) {
				try {
					...
					if (size > MIN_FLUSH_THRESHOLD) {
						// How much should we flush during this iteration? This
						// scheme is
						// adaptive and flushes at a rate that is proportional
						// to the size
						// of the queue, so we flush more aggressively if the
						// queue is big.
						// Let's suppose MAX_TIMESPAN = 1h. We have `size' rows
						// to compact,
						// and we better compact them all in less than 1h,
						// otherwise we're
						// going to "fall behind" when after a new hour starts
						// (as we'll be
						// inserting a ton of new rows then). So slice
						// MAX_TIMESPAN using
						// FLUSH_INTERVAL to compute what fraction of `size' we
						// need to
						// flush at each iteration. Note that `size' will
						// usually account
						// for many rows that can't be flushed yet (not old
						// enough) so we're
						// overshooting a bit (flushing more aggressively than
						// necessary).
						// This isn't a problem at all. The only thing that
						// matters is that
						// the rate at which we flush stuff is proportional to
						// how much work
						// is sitting in the queue. The multiplicative factor
						// FLUSH_SPEED
						// is added to make flush even faster than we need. For
						// example, if
						// FLUSH_SPEED is 2, then instead of taking 1h to flush
						// what we have
						// for the previous hour, we'll take only 30m. This is
						// desirable so
						// that we evict old entries from the queue a bit
						// faster.
						final int maxflushes = Math.max(MIN_FLUSH_THRESHOLD,
								size * FLUSH_INTERVAL * FLUSH_SPEED
										/ Const.MAX_TIMESPAN);
						final long now = System.currentTimeMillis();
						flush(now / 1000 - Const.MAX_TIMESPAN - 1, maxflushes);
						if (LOG.isDebugEnabled()) {
							final int newsize = size();
							LOG.debug("flush() took "
					...
					// Let's free up some memory by throwing away the compaction
					// queue.
					...
					// Catching this kind of error is totally unexpected and is
					// really
					// bad. If we do nothing and let this thread die, we'll run
					// out of
					// memory as new entries are added to the queue. We could
					// always
					// commit suicide, but it's kind of drastic and nothing else
					// in the
					// code does this. If `enable_compactions' wasn't final, we
					// could
					// always set it to false, but that's not an option. So in
					// order to
					// try to get a fresh start, let this compaction thread
					// terminate
					// and spin off a new one instead.
					try {
						Thread.sleep(1000); // Avoid busy looping creating new
											// threads.
					} catch (InterruptedException i) {
						LOG.error(
								"Compaction thread interrupted in error handling",
								i);
				...
				try {
					...
					LOG.error(
							"Compaction thread interrupted, doing one last flush",
							e);
					flush();
					return;
				}
	 ...
	 * <p>
	 * This comparator sorts things by timestamp first, this way we can find all
	 * rows of the same age at once.
	 */
	private static final class Cmp implements Comparator<byte[]> {
		...
		public int compare(final byte[] a, final byte[] b) {
			final int c = Bytes.memcmp(a, b, metric_width,
					Const.TIMESTAMP_BYTES);
			// If the timestamps are equal, sort according to the entire row
			// key.
