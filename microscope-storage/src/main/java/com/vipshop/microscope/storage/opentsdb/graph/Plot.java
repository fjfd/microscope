...
package com.vipshop.microscope.storage.opentsdb.graph;

import com.vipshop.microscope.storage.opentsdb.core.DataPoint;
...
public final class Plot {
     ...
     * @param start_time Timestamp of the start time of the graph.
     * @param end_time   Timestamp of the end time of the graph.
     * @param tz         Timezone to use to render the timestamps. If {@code null} the
     *                   current timezone as of when the JVM started is used.
     * @throws IllegalArgumentException if either timestamp is 0 or negative.
     * @throws IllegalArgumentException if {@code start_time >= end_time}.
     * @since 1.1
     */
     ...
     *
     * @param params Each entry is a Gnuplot setting that will be written as-is in
     *               the Gnuplot script file: {@code set KEY VALUE}. When the value
     *               is {@code null} the script will instead contain
     *               {@code unset KEY}.
     *               <p/>
     *               Special parameters with a special meaning (since OpenTSDB
     *               1.1):
     *               <ul>
     ...
     * @throws IllegalArgumentException if the width or height are negative, zero or "too small"
     ...
     *
     * @param basepath The base path to use. A number of new files will be created
        ...
        for (int i = 0; i < nseries; i++) {
            ...
            try {
                for (final DataPoint d : datapoints.get(i)) {
                    ...
                    if (ts >= (start_time & UNSIGNED)
                            && ts <= (end_time & UNSIGNED)) {
                        npoints++;
                    }
     ...
     * @param datafiles The names of the data files that need to be plotted, in the
    ...
    private void writeGnuplotScript(final String basepath,
        ...
        try {
            ...
            gp.append(
                    ...
                    .append("\"\n" + "set xrange [\"")
                    .append(String
                            .valueOf((start_time & UNSIGNED) + utc_offset))
                    .append("\":\"")
