...
final class SpanGroup implements DataPoints {
     ...
     *                    Ignored if {@code null}. Additional spans can be added with
     *                    {@link #add}.
     ...
     * @param downsampler Aggregation function to use to group data points within an
     *                    interval.
     */
    SpanGroup(final TSDB tsdb, final long start_time, final long end_time,
              final Iterable<Span> spans, final boolean rate,
              final Aggregator aggregator, final int interval,
     ...
     *
     * @param span The span to add to this group. If none of the data points fall
     *             within our time range, this method will silently ignore that
     *             span.
     */
    void add(final Span span) {
        if (tags != null) {
            throw new AssertionError(
                    "The set of tags has already been computed"
    ...
    /**
     * Computes the intersection set + symmetric difference of tags in all
     * spans.
     *
    ...
    private String toStringSharedAttributes() {
        return "start_time=" + start_time + ", end_time=" + end_time
                + ", tags=" + tags + ", aggregated_tags=" + aggregated_tags
                + ", rate=" + rate + ", aggregator=" + aggregator
                + ", downsampler=" + downsampler + ", sample_interval="
                + sample_interval + ')';
    }
     ...
     * specific timestamp, we do a linear interpolation in order to estimate
     * what the value of that Span should be at that time.
     * <p/>
     ...
     * time T1. Let's note that (V1, T1). Now this group has 2 Spans, which
     * means we're trying to aggregate 2 different series (same metric ID but
     * different tags). So The next value that this iterator returns needs to be
     * a combination of V1 and V2 (assuming that T2 is less than T1). If our
     ...
     * less than or equal to T1. If T2 is greater than T1, we ignore V2 and
     * return just V1, since we haven't reached the time yet where V2 exist, so
     * it's essentially as if it wasn't there. Say T2 is less than T1. Summing
     * up V1 and V2 doesn't make sense, since they represent two measurements
     * made at different times. So instead, we need to find what the value V2
     * would have been, had it been measured at time T1 instead of T2. We do
     * this using linear interpolation between the data point (V2, T2) and the
     * following one for that series, (V4, T4). The result is thus the sum of V1
     * and the interpolated value between V2 and V4.
     * <p/>
     ...
     * the arrays depicted above), the timestamp will be set to 0. When there is
     * no "next" value (2nd half of the arrays), the timestamp will be set to a
     * special, really large value (too large to be a valid timestamp).
     * <p/>
    ...
    private final class SGIterator implements SeekableView, DataPoint,
         ...
         * pre-load the current and the next values into the {@link #timestamps}
         * and {@link #values} member. Once we reach the end of a Span, we'll
         * null out its iterator from this array.
         */
         ...
         * in {@code timestamps[iterators.length * 2 + i]}.</li>
         * </ul>
         * <p/>
         * Each timestamp can have the {@code FLAG_FLOAT} applied so it's
         * important to use the {@code TIME_MASK} when getting the actual
         * timestamp value out of it. There are two special values for
         * timestamps:
         * <ul>
        ...
        /**
         * The current and next values for the data points being used. This
         * array works exactly in the same fashion as the 'timestamps' array.
         * This array is also used to store floating point values, in which case
         * their binary representation just happens to be stored in a
         * {@code long}.
         */
        ...
        SGIterator() {
            ...
            for (int i = 0; i < size; i++) {
                final SeekableView it = (downsampler == null ? spans.get(i)
                        .spanIterator() : spans.get(i).downsampler(
        ...
        public double nextDoubleValue() {
            if (hasNextValue(true)) {
                ...
                if (rate) {
                    ...
                    assert x0 > x1 : ("Next timestamp (" + x0
                            + ") is supposed to be "
                ...
                final long x0 = timestamps[pos] & TIME_MASK;
                if (x == x0) {
                    // LOG.debug("No lerp needed x == x0 (" + x +
                    // " == "+x0+") => " + y0);
                    return y0;
                }
                ...
                if (x == x1) {
                    // LOG.debug("No lerp needed x == x1 (" + x +
                    // " == "+x1+") => " + y1);
                    return y1;
                }
        ...
        public String toString() {
            return "SpanGroup.Iterator(timestamps="
                    + Arrays.toString(timestamps) + ", values="
                    + Arrays.toString(values) + ", current=" + current
                    + ", pos=" + pos + ", (SpanGroup: "
