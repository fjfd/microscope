...
package com.vipshop.microscope.storage.opentsdb.stats;
 ...
 * <p>
 * This is not a general purpose implementation of histogram. It's specifically
 * designed for "small" values (close to 0) as the primary use case is latency
 * histograms.
 * <p>
 * All values must be positive ({@code >= 0}).
 * <p>
 * The histogram is linear (fixed size buckets) up to a given cutoff point.
 * Beyond that point, the histogram becomes exponential (each bucket is twice as
 * large as the previous one). This gives good granularity for lower values
 * while still allowing a rough classification for the "long tail" of larger
 * values.
 * <p>
 * Note that this implementation doesn't allow you to directly control the
 * number of buckets in the histogram. The number will depend on the arguments
...
public final class Histogram {
	...
	/**
	 * How many linear buckets we have. Technically we don't need to store this
	 * value but we do in order to avoid having to re-compute it in the fast
	 * path each time we add a new value.
	 */
	private final short num_linear_buckets;

	/**
	 * The power of 2 used by the first exponential bucket. Technically we don't
	 * need to store this value but we do in order to avoid having to re-compute
	 * it in the fast path each time we add a new value.
	 */
	 ...
	 * 
	 * @param max
	 *            The maximum value of the histogram. Any value greater than
	 *            this will be considered to be "infinity".
	 * @param interval
	 *            The interval (size) of each linear bucket.
	 * @param cutoff
	 *            The value beyond which to switch to exponential buckets. The
	 *            histogram may actually use this value or a value up to
	 *            {@code interval} greater.
	 * @throws IllegalArgumentException
	 *             if any of following conditions are not met:
	 * 
	 *             <pre>
	 *   0 &lt; interval &lt;= max
	 *   0 &lt;= cutoff &lt;= max
	 * </pre>
	 */
	public Histogram(final int max, final short interval, final int cutoff) {
		if (interval > max) {
			throw new IllegalArgumentException("interval > max! interval="
					+ interval + ", max=" + max);
		} else if (cutoff > max) {
			throw new IllegalArgumentException("cutoff > max! cutoff=" + cutoff
					+ ", max=" + max);
		} else if (interval < 1) {
			throw new IllegalArgumentException("interval < 1! interval="
					+ interval);
		} else if (cutoff < 0) {
	 ...
	 * {@code Math.ceil(Math.log(n) / Math.log(2))} except it's 3 times faster.
	 * 
	 * @param n
	 *            A strictly positive integer.
	 * @return The logarithm base 2. As a special case, if the integer given in
	 *         argument is 0, this function returns 0. If the integer given in
	 *         argument is negative, the return value is undefined.
	 * @see #log2rounddown
	 */
	 ...
	 * <p>
	 * This is essentially equivalent to
	 * {@code Math.floor(Math.log(n) / Math.log(2))} except it's 4.5 times
	 * faster. This function is also almost 70% faster than {@link #log2roundup}.
	 * 
	 * @param n
	 *            A strictly positive integer.
	 * @return The logarithm base 2. As a special case, if the integer given in
	 ...
	 *            The value to add.
	 * @throws IllegalArgumentException
	 *             if the value given is negative.
	 */
	 ...
	 * 
	 * @param p
	 *            A strictly positive integer in the range {@code [1; 100]}
	 * @throws IllegalArgumentException
	 *             if {@code p} is not valid.
	 */
	public int percentile(int p) {
		...
		p = count * p / 100;
		// Now walk the array backwards and decrement the count until it reaches
		// p.
	 ...
	 * <p>
	 * This is equivalent to calling {@link #printAsciiBucket} on every bucket.
	 * 
	 * @param out
	...
	private int bucketIndexFor(final int value) {
		...
		int bucket = num_linear_buckets // Skip all linear buckets.
				...
				// Reminder: the first exponential bucket ends at
				// 2^exp_bucket_shift.
	...
	private int bucketLowInterval(final int index) {
		if (index <= num_linear_buckets) {
			...
			return cutoff
					+ (1 << (index - num_linear_buckets + exp_bucket_shift));
		}
	}
	...
	public String toString() {
		return "Histogram(interval=" + interval + ", cutoff=" + cutoff
				...
				+ ", exp_bucket_shift=" + exp_bucket_shift + ", buckets="
