...
final class IncomingDataPoints implements WritableDataPoints {
	...
	private static final boolean AUTO_METRIC = System
			.getProperty("tsd.core.auto_create_metrics") != null;
	 ...
	 * want buckets up to 16s, with 2 ms interval between each bucket up to 100
	 * ms after we which we switch to exponential buckets.
	 */
	...
	static byte[] rowKeyTemplate(final TSDB tsdb, final String metric,
		...
		copyInRowKey(row, pos,
				(AUTO_METRIC ? tsdb.metrics.getOrCreateId(metric)
	 ...
	 *            The value to store.
	 * @param flags
	 *            Flags to store in the qualifier (size and type of the data
	 *            point).
	 * @return A deferred object that indicates the completion of the request.
	 */
	private Deferred<Object> addPointInternal(final long timestamp,
			final byte[] value, final short flags) {
		// This particular code path only expects integers on 8 bytes or
		// floating
		...
		if (size > 0) {
			...
			// This is the first data point, let's record the starting
			// timestamp.
			base_time = updateBaseTime(timestamp);
			Bytes.setInt(row, (int) base_time, tsdb.metrics.width());
		}
		...
		values[size] = (value.length == 8 ? Bytes.getLong(value) : Bytes
				.getInt(value) & 0x00000000FFFFFFFFL);
		size++;
		...
		// histogram so we can see how the latency distribution varies over
		// time.
	...
	private void grow() {
		...
		if (new_size == size) {
			throw new AssertionError("Can't grow " + this + " larger than "
					+ size);
		}
