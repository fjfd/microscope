...
final class Span implements DataPoints {
	 ...
	 * 
	 * @param row
	 *            The compacted HBase row to add to this span.
	 * @throws IllegalArgumentException
	 *             if the argument and this span are for two different time
	 *             series.
	 * @throws IllegalArgumentException
	 *             if the argument represents a row for data points that are
	 *             older than those already added to this span.
	 */
		...
		if (rows.size() != 0) {
			...
			if (error != null) {
				throw new IllegalArgumentException(error + ". "
						...
						+ Arrays.toString(last.key)
						+ " whereas the row key being added is "
						+ Arrays.toString(key) + " and metric_width="
						+ metric_width);
			}
	 ...
	 * @param row
	 *            A compacted HBase row.
	 * @return A strictly positive 32-bit timestamp.
	 * @throws IllegalArgumentException
	 *             if {@code row} doesn't contain any cell.
	 */
	static long lastTimestampInRow(final short metric_width, final KeyValue row) {
		...
		final short last_delta = (short) (Bytes.getUnsignedShort(qual,
				qual.length - 2) >>> Const.FLAG_BITS);
	...
	/**
	 * Finds the index of the row of the ith data point and the offset in the
	 * row.
	 * 
	 * @param i
	 *            The index of the data point to find.
	 * @return two ints packed in a long. The first int is the index of the row
	 *         in {@code rows} and the second is offset in that {@link RowSeq}
	 *         instance.
	 */
	...
	public String toString() {
		...
		buf.append("Span(").append(rows.size()).append(" rows, [");
	...
	final class Iterator implements SeekableView {
		...
		public String toString() {
			return "Span.Iterator(row_index=" + row_index + ", current_row="
					+ current_row + ", span=" + Span.this + ')';
		}
	 ...
	 * to be efficient.
	 */
	final class DownsamplingIterator implements SeekableView, DataPoint,
		...
		/**
		 * Current timestamp (unsigned 32 bits). The most significant bit is
		 * used to store FLAG_FLOAT.
		 */
		private long time;
		 ...
		 * 
		 * @param interval
		 *            The interval in seconds wanted between each data point.
		 * @param downsampler
		 *            The downsampling function to use.
		 * @param iterator
		 *            The iterator to access the underlying data.
		 */
		DownsamplingIterator(final int interval, final Aggregator downsampler) {
		...
		private boolean moveToNext() {
			if (!current_row.hasNext()) {
				...
				if (row_index < rows.size() - 1) { // Do we have more rows?
					current_row = rows.get(++row_index).internalIterator();
					current_row.next(); // Position the iterator on the first
										// element.
					return true;
		...
		public DataPoint next() {
			if (!hasNext()) {
				throw new NoSuchElementException("no more data points in "
						+ this);
			}

			// Look ahead to see if all the data points that fall within the
			// next
			...
			// Compute `value'. This will rely on `time' containing the end time
			// of
			// this interval...
			if (integer) {
		...
		public void seek(final long timestamp) {
			short row_index = seekRow(timestamp);
			if (row_index != this.row_index) {
				// LOG.debug("seek from row #" + this.row_index + " to " +
				// row_index);
				this.row_index = row_index;
				current_row = rows.get(row_index).internalIterator();
			}
		...
		public boolean hasNextValue() {
			if (!current_row.hasNext()) {
				if (row_index < rows.size() - 1) {
					// LOG.info("hasNextValue: next row? " + (rows.get(row_index
					// + 1).timestamp(0) < time));
			...
			// LOG.info("hasNextValue: next point? " +
			// (current_row.peekNextTimestamp() < time));
		...
		public long nextLongValue() {
			...
			throw new NoSuchElementException("no more longs in interval of "
					+ this);
		}
		...
		public double nextDoubleValue() {
			if (hasNextValue()) {
				moveToNext();
				// Use `toDouble' instead of `doubleValue' because we can get
				// here if
				// there's a mix of integer values and floating point values in
				// the
				// current downsampled interval.
				return current_row.toDouble();
			}
			throw new NoSuchElementException("no more floats in interval of "
					+ this);
		}
		...
		public String toString() {
			...
			buf.append("Span.DownsamplingIterator(interval=").append(interval)
					...
					.append(", current_row=")
					.append(current_row.toStringSummary())
