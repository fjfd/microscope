...
/**
 * Tool to dump the data straight from HBase. Useful for debugging data induced
 * problems.
 */
final class DumpSeries {
	...
	private static void doDump(final TSDB tsdb, final HBaseClient client,
			final byte[] table, final boolean delete,
			final boolean importformat, final String[] args) throws Exception {
		...
		for (final Query query : queries) {
			...
			while ((rows = scanner.nextRows().joinUninterruptibly()) != null) {
				for (final ArrayList<KeyValue> row : rows) {
					...
					// Print the row key.
					if (!importformat) {
						buf.append(Arrays.toString(key)).append(' ')
								.append(metric).append(' ').append(base_time)
								.append(" (").append(date(base_time))
								.append(") ");
						try {
					...
					for (final KeyValue kv : row) {
						...
						formatKeyValue(buf, tsdb, importformat, kv, base_time,
								metric);
						buf.append('\n');
						System.out.print(buf);
					}
	...
	static void formatKeyValue(final StringBuilder buf, final TSDB tsdb,
			final KeyValue kv, final long base_time) {
	...
	private static void formatKeyValue(final StringBuilder buf,
			final TSDB tsdb, final boolean importformat, final KeyValue kv,
			final long base_time, final String metric) {
		if (importformat) {
		...
		if (multi_val) {
			buf.append(Arrays.toString(qualifier)).append(' ')
					.append(Arrays.toString(cell)).append(" = ")
					.append(nvalues).append(" values:");
		}

		final String tags;
		if (importformat) {
			final StringBuilder tagsbuf = new StringBuilder();
			for (final Map.Entry<String, String> tag : Internal.getTags(tsdb,
					kv.key()).entrySet()) {
				tagsbuf.append(' ').append(tag.getKey()).append('=')
						.append(tag.getValue());
			}
		...
		for (int i = 0; i < nvalues; i++) {
			...
			if (importformat) {
				...
				final byte[] v = multi_val ? Arrays.copyOfRange(cell,
						value_offset, value_offset + value_len) : cell;
				buf.append(Arrays.toString(Bytes.fromShort(qual))).append(' ')
						.append(Arrays.toString(v)).append('\t').append(delta)
						.append('\t');
			}
			if ((qual & 0x8) == 0x8) {
				if (cell.length == 8 && value_len == 4 && cell[0] == 0
						&& cell[1] == 0 && cell[2] == 0 && cell[3] == 0) {
					// Incorrect encoded floating point value.
					// See CompactionQueue.fixFloatingPointValue() for more
					// details.
					value_offset += 4;
				}
				buf.append(importformat ? "" : "f ").append(
						Internal.extractFloatingPointValue(cell, value_offset,
								flags));
			} else {
				buf.append(importformat ? "" : "l ")
						.append(Internal.extractIntegerValue(cell,
								value_offset, flags));
			}
			if (importformat) {
				buf.append(tags);
				...
				buf.append('\t').append(base_time + delta).append(" (")
						.append(date(base_time + delta)).append(')');
			}
