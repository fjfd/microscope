...
final class CliQuery {
	...
	public static void main(String[] args) throws IOException {
		...
		if (plot != null) {
			try {
				...
				LOG.error("Failed to write the Gnuplot file under " + basepath,
						e);
				System.exit(1);
			}
		}
				...
				if (want_plot) {
					...
					for (final DataPoint datapoint : datapoints) {
						buf.append(metric).append(' ')
								.append(datapoint.timestamp()).append(' ');
	 ...
	 * @param plotparams
	 *            The list in which global plot parameters will be appended.
	 *            Ignored if {@code null}.
	 * @param plotoptions
	 *            The list in which per-line plot options will be appended.
	 *            Ignored if {@code null}.
	 */
	static void parseCommandLineQuery(final String[] args, final TSDB tsdb,
		...
		while (i < args.length) {
			...
			final Aggregator sampler = downsample ? Aggregators.get(args[i++])
					: null;
