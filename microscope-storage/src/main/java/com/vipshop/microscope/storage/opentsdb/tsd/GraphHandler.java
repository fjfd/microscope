...
import com.vipshop.microscope.storage.opentsdb.graph.Plot;
...
final class GraphHandler implements HttpRpc {
     ...
     * @param query      The query to serve.
     * @param end_time   The end time on the query (32-bit unsigned int, seconds).
     * @param max_age    The maximum time (in seconds) we wanna allow clients to cache
     ...
     * @throws IOException      if the Gnuplot files can't be written, or the Gnuplot
            ...
            if (i > 0) {
                ...
                if (dash < 0) {
                    throw new BadRequestException(
                            "Invalid downsampling specifier '" + parts[1]
                                    + "' in m=" + m
                    );
                }
    ...
    private static long getQueryStringDate(final HttpQuery query,
        ...
        long timestamp;
        if (date.length() < 5 || date.charAt(4) != '/') { // Already a
            // timestamp?
            try {
    ...
    private void doGraph(final TSDB tsdb, final HttpQuery query)
        ...
        if (options == null) {
            ...
            throw new BadRequestException(options.size()
                    + " `o' parameters, but " + tsdbqueries.length
                    + " `m' parameters.");
        }
        ...
        for (int i = 0; i < nqueries; i++) {
            try { // execute the TSDB query!
                // XXX This is slow and will block Netty. TODO(tsuna): Don't
                // block.
    ...
    private boolean isDiskCacheHit(final HttpQuery query, final long end_time,
        ...
        if (cachedfile.exists()) {
            ...
            if (bytes < 21) { // Minimum possible size for a PNG: 21 bytes.
                // For .txt files, <21 bytes is almost
                // impossible.
