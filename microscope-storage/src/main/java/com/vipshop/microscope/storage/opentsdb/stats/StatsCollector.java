 ...
 * <p/>
 * Instances of this class are passed around to other classes to collect their
 * stats/metrics and do something with them (presumably send them to a client).
 * <p/>
...
public abstract class StatsCollector {
     ...
     * Constructor.
     *
     * @param prefix A prefix to add to every metric name, for example `tsd'.
     */
     ...
     *
     * @param datapoint A data point in a format suitable for a text import.
     */
     ...
     * @param name    The name of the metric.
     * @param value   The current value for that metric.
     * @param xtratag An extra tag ({@code name=value}) to add to those data points
     *                (ignored if {@code null}).
     * @throws IllegalArgumentException if {@code xtratag != null} and it doesn't follow the
     ...
     *                                  {@code name=value} format.
     */
    public final void record(final String name, final Histogram histo,
                             final String xtratag) {
     ...
     * @param name    The name of the metric.
     * @param value   The current value for that metric.
     * @param xtratag An extra tag ({@code name=value}) to add to this data point
     *                (ignored if {@code null}).
     * @throws IllegalArgumentException if {@code xtratag != null} and it doesn't follow the
     *                                  {@code name=value} format.
     */
    public final void record(final String name, final long value,
                             final String xtratag) {
        buf.setLength(0);
        buf.append(prefix).append(".").append(name).append(' ')
                .append(System.currentTimeMillis() / 1000).append(' ')
                .append(value);

        if (xtratag != null) {
            if (xtratag.indexOf('=') != xtratag.lastIndexOf('=')) {
                throw new IllegalArgumentException("invalid xtratag: "
                        + xtratag + " (multiple '=' signs), name=" + name
                        + ", value=" + value);
            } else if (xtratag.indexOf('=') < 0) {
                throw new IllegalArgumentException("invalid xtratag: "
                        + xtratag + " (missing '=' signs), name=" + name
                        + ", value=" + value);
            }
     ...
     * <p/>
     * All subsequent calls to one of the {@code record} methods will associate
     * the tag given to this method with the data point.
     * <p/>
     * This method can be called multiple times to associate multiple tags with
     * all the subsequent data points.
     *
     * @param name  The name of the tag.
     * @param value The value of the tag.
     * @throws IllegalArgumentException if the name or the value are empty or otherwise invalid.
     * @see #clearExtraTag
     */
    public final void addExtraTag(final String name, final String value) {
        if (name.length() <= 0) {
            ...
            throw new IllegalArgumentException("tag value contains `=': "
                    + value + " (name = " + name + ')');
        }
        if (extratags == null) {
     ...
     * current host. If the hostname cannot be looked up, {@code (unknown)} is
     * used instead.
     */
     ...
     *
     * @param name The name of the tag to remove from the set of extra tags.
     * @throws IllegalStateException    if there's no extra tag currently recorded.
     * @throws IllegalArgumentException if the given name isn't in the set of extra tags currently
     *                                  recorded.
     * @see #addExtraTag
     */
    public final void clearExtraTag(final String name) {
        ...
        if (extratags.get(name) == null) {
            throw new IllegalArgumentException("tag '" + name + "' not in"
                    + extratags);
        }
