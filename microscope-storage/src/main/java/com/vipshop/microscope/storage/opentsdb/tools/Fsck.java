...
import com.vipshop.microscope.storage.opentsdb.core.IllegalDataException;
import com.vipshop.microscope.storage.opentsdb.core.Internal;
...
final class Fsck {
	...
	private static int fsck(final TSDB tsdb, final HBaseClient client,
			final byte[] table, final boolean fix, final String[] args)
			throws Exception {

		/** Callback to asynchronously delete a specific {@link KeyValue}. */
		final class DeleteOutOfOrder implements
			...
			public Deferred<Object> call(final Object arg) {
				return client.delete(new DeleteRequest(table, kv.key(), kv
						.family(), kv.qualifier()));
			}
		...
		for (final Query query : queries) {
			...
			while ((rows = scanner.nextRows().joinUninterruptibly()) != null) {
				for (final ArrayList<KeyValue> row : rows) {
					rowcount++;
					// Take a copy of the row-key because we're going to
					// zero-out the
					// timestamp and use that as a key in our `seen' map.
					final byte[] key = row.get(0).key().clone();
					final long base_time = Bytes.getUnsignedInt(key,
							metric_width);
					for (int i = metric_width; i < metric_width
							+ Const.TIMESTAMP_BYTES; i++) {
						key[i] = 0;
					}
					...
					for (final KeyValue kv : row) {
						...
						if (kvcount % 100000 == 0) {
							...
							LOG.info("... " + kvcount + " KV analyzed in "
									+ ping_start_time + "ms ("
						...
						if (qual.length < 2) {
							...
							boolean ooo = false; // Did we find out of order
													// data?
							...
							prev.kv = kv;
							// Check we consumed all the bytes of the value. The
							// last byte
							// is metadata, so it's normal that we didn't
							// consume it.
							if (val_idx != value.length - 1) {
								errors++;
								LOG.error("Corrupted value: consumed "
										+ val_idx
								...
								if (fix) {
									client.put(
											new PutRequest(table,
													ordered.key(), ordered
															.family(), ordered
															.qualifier(),
													ordered.value()))
											.addCallbackDeferring(
													new DeleteOutOfOrder(kv));
								} else {
						...
						} // else: qualifier is on 2 bytes, it's an individual
							// value.
						...
						if ((qualifier & (0x8 | 0x3)) == (0x8 | 0x3)) { // float
																		// | 4
																		// bytes
							// The qualifier says the value is on 4 bytes, and
							// the value is
							// on 8 bytes, then the 4 MSBs must be 0s. Old
							// versions of the
							// code were doing this. It's kinda sad. Some
							// versions had a
							// bug whereby the value would be sign-extended, so
							// we can
							...
							if (value.length == 8) {
								if (value[0] == -1 && value[1] == -1
									...
									correctable++;
									if (fix) {
										value = value.clone(); // We're going to
																// change it.
										value[0] = value[1] = value[2] = value[3] = 0;
										client.put(new PutRequest(table, kv
												.key(), kv.family(), qual,
												value));
									} else {
						...
						if (timestamp <= prev.timestamp()) {
							...
							if (fix) {
								...
								Bytes.setInt(newkey, (int) new_base_time,
										metric_width);
								final short newqual = (short) ((timestamp - new_base_time) << Internal.FLAG_BITS | (qualifier & Internal.FLAGS_MASK));
								final DeleteOutOfOrder delooo = new DeleteOutOfOrder(
										kv);
								if (timestamp < prev.timestamp()) {
									client.put(
											new PutRequest(table, newkey, kv
													.family(), Bytes
													.fromShort(newqual), value))
									// Only delete the offending KV once we're
									// sure that the new
									// KV has been persisted in HBase.
											.addCallbackDeferring(delooo);
								} else {
									// We have two data points at exactly the
									// same timestamp.
									// This can happen when only the flags
									// differ. This is
									// typically caused by one data point being
									// an integer and
									// the other being a floating point value.
									// In this case
									// we just delete the duplicate data point
									// and keep the
									// first one we saw.
									delooo.call(null);
								}
		...
		if (!fix && correctable > 0) {
			System.out.println(correctable
