...
import com.vipshop.microscope.storage.opentsdb.tsd.PipelineFactory;
...
final class TSDMain {
	...
	private static void setDirectoryInSystemProps(final String prop,
			final String dir, final boolean need_write, final boolean create) {
	...
	public static void main(String[] args) {
		...
		if (args == null || !argp.has("--port") || !argp.has("--staticroot")
				|| !argp.has("--cachedir")) {
		...
		setDirectoryInSystemProps("tsd.http.staticroot",
				argp.get("--staticroot"), DONT_CREATE, !MUST_BE_WRITEABLE);
		...
		try {
			...
			final InetSocketAddress addr = new InetSocketAddress(bindAddress,
					Integer.parseInt(argp.get("--port")));
			server.bind(addr);
	 ...
	 *             if the flush interval is negative.
