...
final class RowSeq implements DataPoints {
	 ...
	 * 
	 * @param row
	 *            The compacted HBase row to set.
	 * @throws IllegalStateException
	 *             if this method was already called.
	 */
	void setRow(final KeyValue row) {
		if (this.key != null) {
			throw new IllegalStateException("setRow was already called on "
					+ this);
		}
	 ...
	 * have data points that are close enough together that they could be stored
	 * into the same row, it makes sense to merge them into the same
	 * {@link RowSeq} instance in memory in order to save RAM.
	 * 
	 * @param row
	 *            The compacted HBase row to merge into this instance.
	 * @throws IllegalStateException
	 *             if {@link #setRow} wasn't called first.
	 * @throws IllegalArgumentException
	 *             if the data points in the argument aren't close enough to
	 *             those in this instance time-wise to be all merged together.
	 */
	void addRow(final KeyValue row) {
		if (this.key == null) {
			throw new IllegalStateException("setRow was never called on "
					+ this);
		}
		...
		if (time_adj <= 0) {
			// Corner case: if the time difference is 0 and the key is the same,
			// it
			...
			// asynchbase will transparently re-open the scanner and start
			// scanning
			// from the row key we were on at the time the timeout happened. In
			// that case, the easiest thing to do is to discard everything we
			// know
			// about this row and start over, since we're going to get the full
			// row
			// again anyway.
			if (time_adj != 0 || !Bytes.equals(this.key, key)) {
				throw new IllegalDataException(
						"Attempt to add a row with a base_time=" + base_time
								+ " <= baseTime()=" + baseTime()
								+ "; Row added=" + row + ", this=" + this);
			}
		...
		int last_delta = Bytes.getUnsignedShort(qualifiers,
				qualifiers.length - 2);
		...
		for (int i = 0; i < len; i += 2) {
			...
			final int time_delta = time_adj
					+ ((qualifier & 0xFFFF) >>> Const.FLAG_BITS);
			if (!canTimeDeltaFit(time_delta)) {
				throw new IllegalDataException("time_delta at index " + i
						+ " is too large: " + time_delta + " (qualifier=0x"
						+ Integer.toHexString(qualifier & 0xFFFF)
						+ " baseTime()=" + baseTime() + ", base_time="
						+ base_time + ", time_adj=" + time_adj + ") for " + row
						+ " to be added to " + this);
			}
			if (last_delta >= time_delta) {
				LOG.error("new timestamp = " + (baseTime() + time_delta)
						+ " (index=" + i + ") is < previous="
						+ (baseTime() + last_delta) + " in addRow with row="
						+ row + " in this=" + this);
		...
		// If both the current `values' and the new `val' are single values,
		// then
		...
		assert newvals[newvals.length - 1] == 0 : "Incorrect meta data byte after merge of "
				+ row
	 ...
	 *         separated by the time delta can fit together in the same
	 *         {@link RowSeq}, {@code false} if they're distant enough in time
	 *         that they must go in different {@link RowSeq} instances.
	 */
	 ...
	 * 
	 * @param value
	 *            The contents of a cell in HBase.
	 * @param value_idx
	 *            The offset inside {@code values} at which the value starts.
	 * @param flags
	 *            The flags for this value.
	 * @return The value of the cell.
	 */
	static long extractIntegerValue(final byte[] values, final int value_idx,
			final byte flags) {
		...
		throw new IllegalDataException("Integer value @ " + value_idx
				+ " not on 8/4/2/1 bytes in " + Arrays.toString(values));
	}
	...
	static double extractFloatingPointValue(final byte[] values,
		...
		throw new IllegalDataException("Floating point value @ " + value_idx
				+ " not on 8 or 4 bytes in " + Arrays.toString(values));
	}
	...
	public long longValue(int i) {
		if (!isInteger(i)) {
			throw new ClassCastException("value #" + i + " is not a long in "
					+ this);
		}
	...
	public double doubleValue(int i) {
		if (isInteger(i)) {
			throw new ClassCastException("value #" + i + " is not a float in "
					+ this);
		}
		final Iterator it = new Iterator();
		while (i-- >= 0) {
	...
	final class Iterator implements SeekableView, DataPoint {
		...
		public void seek(final long timestamp) {
			if ((timestamp & 0xFFFFFFFF00000000L) != 0) { // negative or not 32
															// bits
			...
			// LOG.debug("seek to " + timestamp + " -> now=" +
			// toStringSummary());
		}
		...
		public long longValue() {
			if (!isInteger()) {
				throw new ClassCastException("value #" + ((qual_index - 2) / 2)
						+ " is not a long in " + this);
			}
			final byte flags = (byte) qualifier;
