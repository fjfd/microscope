...
public final class Tags {
	 ...
	 * @param tag
	 *            A String of the form "tag=value".
	 * @throws IllegalArgumentException
	 *             if the tag is malformed.
	 * @throws IllegalArgumentException
	 *             if the tag was already in tags with a different value.
	 */
	 ...
	 * @param tags
	 *            The map to populate with the tags parsed out of the first
	 *            argument.
	 * @return The name of the metric.
	 * @throws IllegalArgumentException
	 *             if the metric is malformed.
	 */
	public static String parseWithMetric(final String metric,
		...
		if (metric.charAt(len - 1) != '}') { // "foo{"
			throw new IllegalArgumentException("Missing '}' at the end of: "
					+ metric);
		} else if (curly == len - 2) { // "foo{}"
			return metric.substring(0, len - 2);
		}
		// substring the tags out of "foo{a=b,...,x=y}" and parse them.
		for (final String tag : splitString(
				metric.substring(curly + 1, len - 1), ',')) {
			try {
				parse(tags, tag);
	 ...
	 * @param row
	 *            The row key in which to search the tag name.
	 * @param name
	 *            The name of the tag to search in the row key.
	 * @return The value associated with the given tag name, or null if this tag
	 *         isn't present in this row key.
	 */
	static String getValue(final TSDB tsdb, final byte[] row, final String name)
	...
	static byte[] getValueId(final TSDB tsdb, final byte[] row,
		...
		for (short pos = (short) (tsdb.metrics.width() + Const.TIMESTAMP_BYTES); pos < row.length; pos += name_width
				+ value_width) {
	 ...
	 * 
	 * @param row
	 *            The row key in which to search.
	 * @param offset
	 *            The offset in {@code row} at which to start searching.
	 * @param bytes
	 *            The bytes to search that the given offset.
	 * @return true if {@code bytes} are present in {@code row} at
	 *         {@code offset}, false otherwise.
	 */
	private static boolean rowContains(final byte[] row, short offset,
			final byte[] bytes) {
	 ...
	 * 
	 * @param tsdb
	 *            The TSDB instance to use for Unique ID lookups.
	 * @param row
	 *            The row key from which to extract the tags.
	 * @return A map of tag names (keys), tag values (values).
	 * @throws NoSuchUniqueId
	 *             if the row key contained an invalid ID (unlikely).
	 */
	 ...
	 * function is the opposite of {@link #resolveIds}.
	 * 
	 * @param tsdb
	 *            The TSDB to use for UniqueId lookups.
	 * @param tags
	 *            The tags to resolve.
	 * @return an array of sorted tags (tag id, tag name).
	 * @throws NoSuchUniqueName
	 *             if one of the elements in the map contained an unknown tag
	 *             name or tag value.
	 */
	 ...
	 *            The TSDB to use for UniqueId lookups.
	 * @param tags
	 *            The tags to resolve. If a new tag name or tag value is seen,
	...
	private static ArrayList<byte[]> resolveAllInternal(final TSDB tsdb,
		...
		for (final Map.Entry<String, String> entry : tags.entrySet()) {
			final byte[] tag_id = (create ? tsdb.tag_names.getOrCreateId(entry
					.getKey()) : tsdb.tag_names.getId(entry.getKey()));
			final byte[] value_id = (create ? tsdb.tag_values
					.getOrCreateId(entry.getValue()) : tsdb.tag_values
	...
	/**
	 * Resolves all the tags IDs (name followed by value) into the a map. This
	 * function is the opposite of {@link #resolveAll}.
	 * 
	 * @param tsdb
	 *            The TSDB to use for UniqueId lookups.
	 * @param tags
	 ...
	 * @throws NoSuchUniqueId
	 *             if one of the elements in the array contained an invalid ID.
	 * @throws IllegalArgumentException
	 *             if one of the elements in the array had the wrong number of
	 *             bytes.
	 */
	static HashMap<String, String> resolveIds(final TSDB tsdb,
			final ArrayList<byte[]> tags) throws NoSuchUniqueId {
		final short name_width = tsdb.tag_names.width();
		final short value_width = tsdb.tag_values.width();
		final short tag_bytes = (short) (name_width + value_width);
		final byte[] tmp_name = new byte[name_width];
