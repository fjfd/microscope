...
final class DataPointsIterator implements SeekableView, DataPoint {
	...
	public void seek(final long timestamp) {
		...
		// Do a binary search to find the timestamp given or the one right
		// before.
		short lo = 0;
	...
	public String toString() {
		return "DataPointsIterator(index="
				+ index
				+ (index >= 0 ? ", current type: "
						+ (isInteger() ? "long" : "float") + ", current value="
						+ (isInteger() ? longValue() : doubleValue())
						: " (iteration not started)") + ", dp=" + dp + ')';
	}
