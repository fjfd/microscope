...
package com.vipshop.microscope.storage.opentsdb.uid;
...
public final class UniqueId implements UniqueIdInterface {
     ...
     * @throws IllegalArgumentException if width is negative or too small/large or if kind is an
     *                                  empty string.
     */
    ...
    private void addNameToCache(final byte[] id, final String name) {
        ...
        if (found != null && !found.equals(name)) {
            throw new IllegalStateException("id=" + Arrays.toString(id)
                    + " => name=" + name + ", already mapped to " + found);
        }
    }
    ...
    public byte[] getId(final String name) throws NoSuchUniqueName,
        ...
        if (id != null) {
            ...
            if (id.length != idWidth) {
                throw new IllegalStateException("Found id.length = "
                        + id.length + " which is != " + idWidth
                        + " required for '" + kind() + '\'');
            }
            addIdToCache(name, id);
            addNameToCache(id, name);
    ...
    public byte[] getOrCreateId(String name) throws HBaseException {
        ...
        while (attempt-- > 0) {
            try {
                ...
                LOG.info("Creating an ID for kind='" + kind() + "' name='"
                        + name + '\'');
            }
            ...
            try {
                id = client.atomicIncrement(
                        new AtomicIncrementRequest(table, MAXID_ROW, ID_FAMILY,
                                kind)
                ).joinUninterruptibly();
                row = Bytes.fromLong(id);
                LOG.info("Got ID=" + id + " for kind='" + kind() + "' name='"
                        + name + "'");
                // row.length should actually be 8.
                if (row.length < idWidth) {
                    throw new IllegalStateException("OMG, row.length = "
                            + row.length + " which is less than " + idWidth
                            + " for id=" + id + " row=" + Arrays.toString(row));
                }
                ...
                throw e; // To avoid handling this exception in the next
                // `catch'.
            } catch (Exception e) {
                LOG.error(
                        "WTF?  Unexpected exception type when assigning an ID,"
                                + " ICV on row=" + Arrays.toString(MAXID_ROW)
                                + " column='" + fromBytes(ID_FAMILY) + ':'
                                + kind() + '\'', e
                );
                continue;
            }
            // If we die before the next PutRequest succeeds, we just waste an
            // ID.
            ...
            try {
                final PutRequest reverse_mapping = new PutRequest(table, row,
                        NAME_FAMILY, kind, toBytes(name));
                // We are CAS'ing the KV into existence -- the second argument
                // is how
                // we tell HBase we want to atomically create the KV, so that if
                // there
                // is already a KV in this cell, we'll fail. Technically we
                // could do
                // just a `put' here, as we have a freshly allocated UID, so
                // there is
                // not reason why a KV should already exist for this UID, but
                // just to
                // err on the safe side and catch really weird corruption cases,
                // we do
                // a CAS instead to create the KV.
                if (!client.compareAndSet(reverse_mapping,
                        HBaseClient.EMPTY_ARRAY).joinUninterruptibly()) {
            ...
            try {
                final PutRequest forward_mapping = new PutRequest(table,
                        toBytes(name), ID_FAMILY, kind, row);
                ...
                if (!client.compareAndSet(forward_mapping,
                        HBaseClient.EMPTY_ARRAY).joinUninterruptibly()) {
                    LOG.warn("Race condition: tried to assign ID "
                            + id
                            + " to "
                            + kind()
                            + ":"
        ...
        LOG.error("Failed to assign an ID for kind='" + kind() + "' name='"
                + name + "'", hbe);
        throw hbe;
    }
     ...
     * terms.
     * @throws HBaseException if there was a problem getting suggestions from HBase.
     */
        ...
        try {
            ...
            while ((short) suggestions.size() < MAX_SUGGESTIONS
                ...
                for (final ArrayList<KeyValue> row : rows) {
                    if (row.size() != 1) {
                        LOG.error("WTF shouldn't happen!  Scanner "
                                + scanner
                                + " returned"
                                + " a row that doesn't have exactly 1 KeyValue: "
                                + row);
                        if (row.isEmpty()) {
                            continue;
                        }
                    ...
                    if (cached_id == null) {
                        ...
                        throw new IllegalStateException("WTF?  For kind="
                                + kind() + " name=" + name + ", we have id="
                                + Arrays.toString(cached_id)
                                + " in cache, but just scanned id="
                                + Arrays.toString(id));
                    }
     ...
     * @throws HBaseException           if there was a problem with HBase while trying to update the
     *                                  mapping.
     */
    public void rename(final String oldname, final String newname) {
        final byte[] row = getId(oldname);
        {
            ...
            if (id != null) {
                throw new IllegalArgumentException("When trying rename(\""
                        + oldname + "\", \"" + newname + "\") on " + this
                        + ": new name already" + " assigned ID="
                        + Arrays.toString(id));
            }
        }
        ...
        try {
            final PutRequest forward_mapping = new PutRequest(table, newnameb,
                    ID_FAMILY, kind, row);
        ...
        try {
            final DeleteRequest old_forward_mapping = new DeleteRequest(table,
                    toBytes(oldname), ID_FAMILY, kind);
            ...
            final String msg = "Unexpected exception when trying rename(\""
                    + oldname + "\", \"" + newname + "\") on " + this
                    + ": Failed to remove the" + " old forward mapping for ID="
                    + Arrays.toString(row);
            LOG.error("WTF?  " + msg, e);
    ...
    private byte[] hbaseGet(final byte[] key, final byte[] family)
        ...
        try {
            final ArrayList<KeyValue> row = client.get(get)
                    .joinUninterruptibly();
     ...
     * @param wait     The initial amount of time in ms to sleep for after a failure.
     *                 This amount is doubled after each failed attempt.
     * @throws HBaseException if all the attempts have failed. This exception will be the
     *                        exception of the last attempt.
     */
    private void hbasePutWithRetry(final PutRequest put, short attempts,
                                   short wait) throws HBaseException {
