...
@Repository
public class ServletRepository extends AbstraceRepository {

	public void initialize() {
		super.initialize(ServletReportTable.INDEX_TABLE_NAME, new String[]{ServletReportTable.CF_APP, ServletReportTable.CF_IP});
	...
	public void drop() {
		...
		super.drop(ServletReportTable.INDEX_TABLE_NAME);
	}
...
//	           UUID.randomUUID().getLeastSignificantBits();
	...
	public void saveActiveRequest(final Map<String, Object> counter) {
		hbaseTemplate.execute(ServletReportTable.INDEX_TABLE_NAME, new TableCallback<Map<String, Object>>() {
			@Override
			public Map<String, Object> doInTable(HTableInterface table) throws Throwable {
				...
				p.add(ServletReportTable.BYTE_CF_APP, Bytes.toBytes((String)counter.get("app")), Bytes.toBytes((String)counter.get("app")));
				p.add(ServletReportTable.BYTE_CF_IP, Bytes.toBytes((String)counter.get("ip")), Bytes.toBytes((String)counter.get("ip")));
				table.put(p);
				return counter;
			}
		});
	...
	public List<Map<String, Object>> findAppIP() {
		...
		hbaseTemplate.find(ServletReportTable.INDEX_TABLE_NAME, ServletReportTable.CF_APP, new RowMapper<List<String>>() {
			@Override
			public List<String> mapRow(Result result, int rowNum) throws Exception {
				String[] appQunitifer = getColumnsInColumnFamily(result, ServletReportTable.CF_APP);
		...
		for (Iterator<String> iterator = appList.iterator(); iterator.hasNext();) {
			...
			hbaseTemplate.get(ServletReportTable.INDEX_TABLE_NAME, row, new RowMapper<Map<String, Object>>() {
				@Override
				public Map<String, Object> mapRow(Result result, int rowNum) throws Exception {
					Map<String, Object> appTrace = new HashMap<String, Object>();
					String[] ipQunitifer = getColumnsInColumnFamily(result, ServletReportTable.CF_IP);
