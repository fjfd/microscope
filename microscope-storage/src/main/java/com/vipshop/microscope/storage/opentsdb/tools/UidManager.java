...
package com.vipshop.microscope.storage.opentsdb.tools;
...
import com.vipshop.microscope.storage.opentsdb.uid.NoSuchUniqueId;
import com.vipshop.microscope.storage.opentsdb.uid.NoSuchUniqueName;
import com.vipshop.microscope.storage.opentsdb.uid.UniqueId;

/**
 * Command line tool to manipulate UIDs. Can be used to find or assign UIDs.
 */
final class UidManager {
	...
	public static void main(String[] args) {
		...
		final short idwidth = (argp.has("--idwidth") ? Short.parseShort(argp
				.get("--idwidth")) : 3);
		if (idwidth <= 0) {
	...
	private static int runCommand(final HBaseClient client, final byte[] table,
			final short idwidth, final boolean ignorecase, final String[] args) {
		...
		if (args[0].equals("grep")) {
			...
			if (1 <= nargs && nargs <= 2) {
				...
				try {
					...
					return lookupName(client, table, idwidth, args[nargs - 1],
							kind);
				}
			} else {
				usage("Wrong number of arguments");
	 ...
	 * @param ignorecase
	 *            Whether or not to ignore the case while grepping.
	 * @param args
	 *            Command line arguments ({@code [kind] RE}).
	 * @return The exit status of the command (0 means at least 1 match).
	 */
	private static int grep(final HBaseClient client, final byte[] table,
	 ...
	 *            The row to print.
	 * @param family
	 *            Only cells in this family (if any) will be printed.
	 * @param formard
	 *            If true, this row contains a forward mapping (name to ID).
	 *            Otherwise the row is assumed to contain a reverse mapping (ID
	 *            to name).
	 * @return {@code true} if at least one cell was printed.
	 */
	private static boolean printResult(final ArrayList<KeyValue> row,
			final byte[] family, final boolean formard) {
		...
		for (final KeyValue kv : row) {
			...
			System.out.println(fromBytes(kv.qualifier()) + ' ' + name + ": "
					+ id);
		}
		return printed;
	}
	 ...
	 *            The name of the HBase table to use.
	 * @param idwidth
	 *            Number of bytes on which the UIDs should be.
	 * @param args
	 *            Command line arguments ({@code assign name [names]}).
	 * @return The exit status of the command (0 means success).
	 */
	private static int assign(final HBaseClient client, final byte[] table,
			final short idwidth, final String[] args) {
	 ...
	 * @return The exit status of the command (0 means success).
	 */
	private static int rename(final HBaseClient client, final byte[] table,
			final short idwidth, final String[] args) {
		...
		try {
			...
			LOG.error("error while processing renaming " + oldname + " to "
					+ newname, e);
			return 3;
	...
	private static int fsck(final HBaseClient client, final byte[] table) {
					...
					for (final KeyValue kv : row) {
						...
						if (Bytes.equals(key, MAXID_ROW)) {
							...
							if (Bytes.equals(family, ID_FAMILY)) {
								...
								final String id = uids.name2id
										.put(skey, svalue);
								if (id != null) {
									uids.error(kv, "Duplicate forward " + kind
											+ " mapping: " + skey + " -> " + id
											+ " and " + skey + " -> " + svalue);
								}
								...
								final String name = uids.id2name.put(skey,
										svalue);
								if (name != null) {
									uids.error(kv, "Duplicate reverse " + kind
											+ "  mapping: " + svalue + " -> "
											+ name + " and " + svalue + " -> "
											+ skey);
								}
							...
							if (uids.width == 0) {
								...
								uids.error(kv, "Invalid " + kind
										+ " ID of length " + idwidth
										+ " (expected: " + uids.width + ')');
							}
						}
					}
		...
		for (final Map.Entry<String, Uids> entry : name2uids.entrySet()) {
			...
			// Look for forward mappings without the corresponding reverse
			// mappings.
			// These are harmful and shouldn't exist.
			for (final Map.Entry<String, String> nameid : uids.name2id
					.entrySet()) {
				final String name = nameid.getKey();
				final String id = nameid.getValue();
				final String found = uids.id2name.get(id);
				if (found == null) {
					uids.error("Forward " + kind
							+ " mapping is missing reverse" + " mapping: "
							+ name + " -> " + id);
				} else if (!found.equals(name)) {
					uids.error("Forward " + kind + " mapping " + name + " -> "
							+ id + " is different than reverse mapping: " + id
							+ " -> " + found);
			...
			// Look for reverse mappings without the corresponding forward
			// mappings.
			// These are harmless but shouldn't frequently occur.
			for (final Map.Entry<String, String> idname : uids.id2name
					.entrySet()) {
			...
			final int maxsize = Math.max(uids.id2name.size(),
					uids.name2id.size());
			if (uids.maxid > maxsize) {
				LOG.warn("Max ID for " + kind + " is " + uids.maxid
						+ " but only " + maxsize
						+ " entries were found.  Maybe "
						+ (uids.maxid - maxsize) + " IDs were deleted?");
			} else if (uids.maxid < maxsize) {
				uids.error("We found " + maxsize + ' ' + kind
	 ...
	 *            The ID to look for.
	 * @param kind
	 *            The 'kind' of the ID (can be {@code null}).
	 * @return The exit status of the command (0 means at least 1 found).
	 */
	private static int lookupId(final HBaseClient client, final byte[] table,
			final short idwidth, final long lid, final String kind) {
	 ...
	 *            The name of the HBase table to use.
	 * @param key
	 *            The row key to attempt to get from HBase.
	 * @param family
	 *            The family in which we're interested.
	...
	private static int findAndPrintRow(final HBaseClient client,
			final byte[] table, final byte[] key, final byte[] family,
			boolean formard) {
	 ...
	 * @param lid
	 *            The ID to transform.
	 * @return The ID represented in {@code idwidth} bytes, or {@code null} if
	 *         {@code lid} couldn't fit in {@code idwidth} bytes.
	 */
	 ...
	 * @return The exit status of the command (0 means at least 1 found).
	 */
	private static int lookupName(final HBaseClient client, final byte[] table,
			final short idwidth, final String name, final String kind) {
		if (kind != null) {
