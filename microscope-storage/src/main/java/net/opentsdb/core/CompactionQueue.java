...
final class CompactionQueue extends ConcurrentSkipListMap<byte[], Boolean> {
  ...
  /**
   * Compacts a row into a single {@link org.hbase.async.KeyValue}.
   ...
   * a {@link org.hbase.async.KeyValue} containing the compacted form of this row.
   ...
   * @return a {@link org.hbase.async.KeyValue} containing the result of the merge of all the
  ...
  /**
   * Breaks down all the values in a row into individual {@link net.opentsdb.core.CompactionQueue.Cell}s.
   * @param row The row to compact.  Assumed to have 2 elements or more.
   * @param estimated_nvalues Estimate of the number of values to compact.
   * Used to pre-allocate a collection of the right size, so it's better to
   * overshoot a bit to avoid re-allocations.
