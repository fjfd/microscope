...
final class IncomingDataPoints implements WritableDataPoints {
  ...
  private static final boolean AUTO_METRIC = true;
  ...
  private Deferred<Object> addPointInternal(final long timestamp, final byte[] value,
    ...
    if ((timestamp & 0xFFFFFFFF00000000L) != 0) {
      ...
      throw new IllegalArgumentException((timestamp < 0 ? "negative " : "bad")
          + " timestamp=" + timestamp
          + " when trying to save value=" + Arrays.toString(value) + " to " + this);
    }
    ...
    if (size > 0) {
      ...
      if (timestamp <= last_ts) {
        throw new IllegalArgumentException("New timestamp=" + timestamp
            ...
            + " when trying to save value=" + Arrays.toString(value)
            + " to " + this);
    ...
    //    putlatency.save((int) ((System.nanoTime() - start_put) / 1000000));
    //    return arg;
    //  }
